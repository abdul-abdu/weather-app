...
let urlForecast: string = "data/2.5/onecall?";

let urlCity: string = "data/2.5/forecast?q=";

export const getCities = async (query: string) =>
	await httpClient.get(`${citiesEndpoint}&q=${query}`);
...
export const getCity = async (query: IQueryCity) =>
	httpClient.get(`${urlCity}${query.city},${query.country}&appid=${apiKey}`);

export const getWeather = async (city: any) =>
	httpClient.get(
		`${weatherEndpoint}&lat=${city.coord.lat}&lon=${city.coord.lon}`
	);
